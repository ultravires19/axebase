...
impl From<crate::utils::token::TokenError> for AppError {
    fn from(err: crate::utils::token::TokenError) -> Self {
        match err {
            crate::utils::token::TokenError::TokenNotFound => {
                AppError::Token("Token not found or invalid".to_string())
            }
            crate::utils::token::TokenError::TokenExpired => {
                AppError::Token("Token has expired".to_string())
            }
            crate::utils::token::TokenError::Database(db_err) => {
                AppError::Database(DbError::Sqlx(db_err))
            }
            crate::utils::token::TokenError::InvalidFormat => {
                AppError::Validation("Invalid token format".to_string())
            }
            crate::utils::token::TokenError::Other(msg) => {
